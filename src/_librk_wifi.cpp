...
void WiFi::init(const rkConfig& cfg) {
    ...
    const bool enable_ble = (
        cfg.rbcontroller_app_enable &&
        !has_station_name && !cfg.wifi_default_ap &&
        (!has_ap_pass || strcmp(cfg.wifi_ap_password, RK_DEFAULT_WIFI_AP_PASSWORD) == 0)
    );
#else
